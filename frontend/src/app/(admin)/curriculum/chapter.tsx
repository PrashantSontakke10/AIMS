...
export default function ChapterDetail() {
  ...
  const handleLectureSubmit = async () => {
    ...
    try {
      if (editingLecture) {
        ...
        setLectures(prev => prev.map(l => (l._id === editingLecture._id ? response.data : l)));
      ...
      Alert.alert('Error', e.response?.data?.message || 'Failed to save lecture');
    } finally {
      ...
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert('Error', e.response?.data?.message || 'Failed to delete lecture');
          }
        },
      },
    ]);
  ...
  const handleNoteSubmit = async () => {
    ...
    if (selectedFiles.length === 0) {
      Alert.alert('Validation Error', 'Please select at least one document file');
      return;
    }
    ...
    try {
      ...
      formData.append('chapterId', chapterId as string);
      ...
      Alert.alert('Success', 'Notes uploaded and sync\'d with Google Drive successfully!');
    } catch (e: any) {
      Alert.alert('Upload Error', e.response?.data?.message || 'Failed to upload notes');
    } finally {
  ...
  const handleOpenNote = async (url: string) => {
    try {
      ...
      Linking.openURL(url).catch(() => Alert.alert('Error', 'Unable to view note URL'));
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabsRow}>
        <TouchableOpacity
          style={[styles.tabButton, activeTab === 'lectures' ? styles.tabButtonActive : null]}
          onPress={() => setActiveTab('lectures')}
        >
          <Text style={[styles.tabButtonText, activeTab === 'lectures' ? styles.tabButtonTextActive : null]}>
            Lectures ({lectures.length})
          </Text>
        </TouchableOpacity>

        <TouchableOpacity
          style={[styles.tabButton, activeTab === 'notes' ? styles.tabButtonActive : null]}
          onPress={() => setActiveTab('notes')}
        >
          <Text style={[styles.tabButtonText, activeTab === 'notes' ? styles.tabButtonTextActive : null]}>
      ...
      <View style={styles.actionRow}>
        <Text style={styles.countIndicatorText}>
          {activeTab === 'lectures' ? `📹 ${lectures.length} Lectures` : `📄 ${notes.length} Notes`}
        </Text>
        <TouchableOpacity
          style={styles.addBtn}
          onPress={activeTab === 'lectures' ? handleOpenCreateLecture : () => setNoteModalVisible(true)}
        >
      ...
      {activeTab === 'lectures' ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.itemCard}>
              ...
              <View style={styles.cardActions}>
                <TouchableOpacity style={styles.actionIconBtn} onPress={() => handleOpenEditLecture(item)}>
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <Text style={styles.emptyText}>No lectures added to this chapter yet.</Text>
            </View>
          }
        />
      ) : (
        ...
        <FlatList
            ...
            <View style={styles.itemCard}>
              <View style={styles.itemDetails}>
                ...
                <View style={styles.linksRow}>
                  <TouchableOpacity
                    style={[styles.actionLinkBtn, { marginRight: Spacing.three }]}
              ...
              <View style={styles.cardActions}>
                <TouchableOpacity style={styles.actionIconBtn} onPress={() => handleDeleteNote(item._id)}>
                  <Text style={styles.actionEmoji}>🗑️</Text>
                </TouchableOpacity>
              </View>
            </View>
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <Text style={styles.emptyText}>No handouts or PDF notes uploaded yet.</Text>
            </View>
          }
        />
      )}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity style={styles.closeBtn} onPress={() => setLectureModalVisible(false)}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity style={styles.closeBtn} onPress={() => setNoteModalVisible(false)}>
                <Text style={styles.closeBtnText}>✕</Text>
              </TouchableOpacity>
            </View>
            ...
            <ScrollView contentContainerStyle={styles.modalBody}>
              ...
              <View style={styles.fileSelectContainer}>
                <TouchableOpacity style={styles.fileSelectBtn} onPress={handleSelectFiles}>
                  <Text style={styles.fileSelectBtnText}>📎 Select PDF or Word Files</Text>
                </TouchableOpacity>

                {selectedFiles.length > 0 ? (
                  <View style={styles.selectedFilesList}>
                    <Text style={styles.selectedHeading}>Selected Files ({selectedFiles.length}):</Text>
                    {selectedFiles.map((file, idx) => (
                      <Text key={idx} style={styles.fileNameText} numberOfLines={1}>
                        📄 {file.name} ({Math.round(file.size ? file.size / 1024 : 0)} KB)
                      </Text>
                    ))}
                  </View>
                ) : (
                  <Text style={styles.noFilesSelectedText}>No documents selected.</Text>
                )}
              </View>
              ...
              >
                {noteSubmitLoading ? (
                  ...
                  <Text style={styles.submitBtnText}>Upload & Sync to Drive</Text>
                )}
              </TouchableOpacity>
            </ScrollView>
          </View>
        ...
        <View style={styles.videoPlayerOverlay}>
          <View style={styles.videoPlayerContent}>
            <View style={styles.videoPlayerHeader}>
              ...
              <TouchableOpacity style={styles.closeBtn} onPress={() => setVideoPlayerVisible(false)}>
