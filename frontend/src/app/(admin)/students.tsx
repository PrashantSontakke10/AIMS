...
export default function StudentsManagement() {
  ...
  const [selectedCoursesForBulk, setSelectedCoursesForBulk] = useState<string[]>([]);
  ...
  const handleApprove = async (userId: string) => {
    try {
      ...
      if (selectedStudent?._id === userId) {
        setSelectedStudent(prev => prev ? { ...prev, status: 'active' as const } : null);
      }
      Alert.alert('Success', 'Student approved successfully');
    } catch (e: any) {
      Alert.alert('Error', e.response?.data?.message || 'Failed to approve student');
    }
  };
  ...
  const handleBlock = async (userId: string) => {
    try {
      ...
      setStudents(prev =>
        prev.map(s => (s._id === userId ? { ...s, status: 'blocked' as const } : s))
      );
      if (selectedStudent?._id === userId) {
        setSelectedStudent(prev => prev ? { ...prev, status: 'blocked' as const } : null);
      }
      Alert.alert('Success', 'Student blocked successfully');
    } catch (e: any) {
      Alert.alert('Error', e.response?.data?.message || 'Failed to block student');
    }
  };

  const handleAssignSingleCourse = async (courseId: string) => {
    ...
    try {
      ...
      const fullAssignedCourses = courses.filter(c => updatedUser.assignedCourses.includes(c._id));
      
      setStudents(prev =>
        prev.map(s => (s._id === selectedStudent._id ? { ...s, status: updatedUser.status, assignedCourses: fullAssignedCourses } : s))
      );
      setSelectedStudent(prev => prev ? { ...prev, status: updatedUser.status, assignedCourses: fullAssignedCourses } : null);
      
      setAssignModalVisible(false);
      Alert.alert('Success', 'Course assigned successfully');
    } catch (e: any) {
      Alert.alert('Error', e.response?.data?.message || 'Failed to assign course');
    }
  };
  ...
  const handleRemoveCourse = async (courseId: string) => {
    ...
    try {
      ...
      setStudents(prev =>
        prev.map(s => (s._id === selectedStudent._id ? { ...s, assignedCourses: fullAssignedCourses } : s))
      );
      setSelectedStudent(prev => prev ? { ...prev, assignedCourses: fullAssignedCourses } : null);
      
      Alert.alert('Success', 'Course removed successfully');
    } catch (e: any) {
      Alert.alert('Error', e.response?.data?.message || 'Failed to remove course');
    }
  };
  ...
  const handleBulkAssign = async () => {
    ...
    try {
      ...
      Alert.alert('Error', e.response?.data?.message || 'Failed to assign bulk courses');
    }
  };

  const toggleBulkCourseSelection = (courseId: string) => {
  ...
  const filteredStudents = students.filter(
    (s) => s.status === activeTab && s.mobile.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabsRow}>
        {(['pending', 'active', 'blocked'] as const).map((tab) => (
          <TouchableOpacity
            key={tab}
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              ...
              <ScrollView contentContainerStyle={styles.modalBody}>
                <Text style={styles.profileMobile}>{selectedStudent.mobile}</Text>
                
                <View style={styles.profileStatusRow}>
                  <Text style={styles.boldLabel}>Status:</Text>
                  <View style={[styles.badge, { backgroundColor: `${Colors[selectedStudent.status]}15` }]}>
                    <Text style={[styles.badgeText, { color: Colors[selectedStudent.status] }]}>
                ...
                <View style={styles.coursesSection}>
                  <View style={styles.coursesHeader}>
                    <Text style={styles.sectionHeaderTitle}>Assigned Courses</Text>
                    <TouchableOpacity
                    ...
                    >
                      <Text style={styles.addCourseBtnText}>+ Assign Single</Text>
                    </TouchableOpacity>
                  </View>
                    ...
                    selectedStudent.assignedCourses.map((c) => (
                      <View key={c._id} style={styles.courseRow}>
                        <View style={styles.courseDetails}>
                          ...
                          {c.code ? <Text style={styles.courseRowCode}>{c.code}</Text> : null}
                        </View>
                        <TouchableOpacity
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      setSelectedCoursesForBulk(selectedStudent.assignedCourses.map(c => c._id));
                      setAssignModalVisible(true);
                    }}
                  >
                    <Text style={styles.bulkAssignTriggerBtnText}>📦 Bulk Assign & Auto-Approve</Text>
                  </TouchableOpacity>
                </View>
              </ScrollView>
            </View>
          ...
          <View style={[styles.modalContent, { maxHeight: '80%' }]}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>
                {bulkAssignMode ? 'Bulk Assign Courses' : 'Select Course to Assign'}
              </Text>
            ...
            <ScrollView contentContainerStyle={styles.modalBody}>
              {courses.map((course) => {
                const isAssigned = selectedStudent?.assignedCourses.some(c => c._id === course._id);
                const isSelectedInBulk = selectedCoursesForBulk.includes(course._id);
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      bulkAssignMode
                        ? isSelectedInBulk ? styles.courseOptionCardSelected : null
                        : isAssigned ? styles.courseOptionCardDisabled : null
                    ]}
                  ...
                  >
                    <View style={styles.courseOptionDetails}>
                      ...
                      {course.code ? <Text style={styles.courseOptionCode}>{course.code}</Text> : null}
                    </View>
                    {bulkAssignMode ? (
                      <View style={[styles.checkbox, isSelectedInBulk ? styles.checkboxChecked : null]}>
                        {isSelectedInBulk && <Text style={styles.checkboxCheckmark}>✓</Text>}
                      </View>
                    ) : (
                      isAssigned && <Text style={styles.assignedBadgeText}>Assigned</Text>
                    )}
                  </TouchableOpacity>
                );
              })}
            ...
            {bulkAssignMode && (
              <View style={styles.bulkSubmitContainer}>
                <TouchableOpacity style={styles.bulkSubmitBtn} onPress={handleBulkAssign}>
                  <Text style={styles.btnTextLight}>Save & Approve Student</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>
